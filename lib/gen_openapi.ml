...
let rec yaml_schema indent_n = function
  ...
  | Ast.Set t ->
    [yi indent_n "type: array"; yi indent_n "uniqueItems: true"; yi indent_n "items:"] @ yaml_schema (indent_n + 1) t
  | Ast.Tuple _ -> [yi indent_n "type: array"]
  | Ast.Map (_, v) ->
    [yi indent_n "type: object"; yi indent_n "additionalProperties:"] @ yaml_schema (indent_n + 1) v
...
let generate (file : Ast.file) : string =
  ...
  List.iter (function
    | Ast.Service s ->
      let base = match Emitter.find_annotation "rest" s.annotations with
        | Some a -> (match Emitter.annotation_string_arg "base" a with
          | Some b -> b
          | None -> (match Emitter.annotation_string_arg "" a with
            | Some b -> b
            | None -> "/" ^ String.lowercase_ascii s.name))
        | None -> "/" ^ String.lowercase_ascii s.name
      in
      List.iter (fun (m : Ast.method_decl) ->
        let (http_method, path) = match m.name with
          | "create" -> ("post", base)
          | "list" -> ("get", base)
          | "get" -> ("get", base ^ "/{id}")
          | "update" -> ("put", base ^ "/{id}")
  ...
  let ordered_paths = List.rev !path_order in
    ...
    List.iter (fun path ->
      ...
      List.iter (fun op ->
        ...
        if m.params <> [] then begin
          let id_params = List.filter (fun (p : Ast.param) ->
            match p.typ with Ast.Ref _ -> true | _ -> false
          ) m.params in
          let body_params = List.filter (fun (p : Ast.param) ->
            match p.typ with Ast.Ref _ -> false | _ -> true
          ) m.params in
